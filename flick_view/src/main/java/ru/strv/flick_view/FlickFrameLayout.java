...
import ru.strv.flick_view.utils.AnimUtils;
import ru.strv.flick_view.utils.ColorUtils;
import ru.strv.flick_view.utils.ViewUtil;
...
public class FlickFrameLayout extends FrameLayout {
    ...
    public FlickFrameLayout(Context context, AttributeSet attrs,
        ...
        final TypedArray a = getContext().obtainStyledAttributes(
                attrs, R.styleable.FlickFrameLayout, 0, 0);

        if (a.hasValue(R.styleable.FlickFrameLayout_dragDismissDistance)) {
            dragDismissDistance = a.getDimensionPixelSize(R.styleable
                    .FlickFrameLayout_dragDismissDistance, 0);
        } else if (a.hasValue(R.styleable.FlickFrameLayout_dragDismissFraction)) {
            dragDismissFraction = a.getFloat(R.styleable
                    .FlickFrameLayout_dragDismissFraction, dragDismissFraction);
        }
        if (a.hasValue(R.styleable.FlickFrameLayout_dragDismissScale)) {
            dragDismissScale = a.getFloat(R.styleable
                    .FlickFrameLayout_dragDismissScale, dragDismissScale);
            shouldScale = dragDismissScale != 1f;
        }
        if (a.hasValue(R.styleable.FlickFrameLayout_dragElasticity)) {
            dragElacticity = a.getFloat(R.styleable.FlickFrameLayout_dragElasticity,
                    dragElacticity);
        }
        a.recycle();
    }
    ...
    public static class SystemChromeFader extends ElasticDragDismissCallback {
        ...
        public SystemChromeFader(Activity activity) {
            ...
            fadeNavBar = ViewUtil.isNavBarOnBottom(activity);
        }
